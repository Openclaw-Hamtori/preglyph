...
export function createInscriptionDataUrl(text, size = MATRIX_SIZE, mode = 'base') {
  ...
  const glyphs = cells
    .map((char, index) => {
      ...
      if (mode === 'hover-fill') {
        return `<text x="${x.toFixed(2)}" y="${y.toFixed(2)}" text-anchor="middle" dominant-baseline="middle" font-family="${INSCRIPTION_FONT_STACK.replace(/"/g, '&quot;')}" font-size="${fontSize}" font-weight="600" fill="rgba(208,236,255,0.84)">${safeChar}</text>`;
      }
