  ...
  const glyphs = cells
    .map((char, index) => {
      ...
      const safeChar = char
        ...
        .replace(/>/g, '&gt;');
      if (mode === 'glow') {
        return `<text x="${x.toFixed(2)}" y="${y.toFixed(2)}" text-anchor="middle" dominant-baseline="middle" font-family="${INSCRIPTION_FONT_STACK.replace(/"/g, '&quot;')}" font-size="${fontSize}" font-weight="600" fill="rgba(196,228,255,0.58)" filter="url(#preglyphGlow)">${safeChar}</text>`;
      }
      if (mode === 'hover-fill') {
