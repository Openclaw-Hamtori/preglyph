...
export function isChainlinkPriceStaleError(error) {
  return /Chainlink ETH\/USD feed is stale/i.test(String(error?.message || error || ''));
}

export async function fetchChainlinkEthUsdPrice({ rpcUrl, feedAddress = DEFAULT_CHAINLINK_ETH_USD_FEED, chainId = 1, maxAgeSeconds = DEFAULT_FEE_QUOTE_TTL_SECONDS, allowStale = false }) {
  if (!rpcUrl) {
  ...
  try {
    ...
    if (!allowStale || !isChainlinkPriceStaleError(error)) {
      throw error;
    }
  }
  return {
    ...
    stale: Number(updatedAt || 0) + Number(maxAgeSeconds || DEFAULT_FEE_QUOTE_TTL_SECONDS) < Math.floor(Date.now() / 1000),
  };
}

export async function getWriteFeeQuote({ rpcUrl, usdCents, overrideWei = '', feedAddress = DEFAULT_CHAINLINK_ETH_USD_FEED, chainId = 1, maxAgeSeconds = DEFAULT_FEE_QUOTE_TTL_SECONDS, allowStale = false }) {
  if (overrideWei) {
    return {
  ...
  const price = await fetchChainlinkEthUsdPrice({ rpcUrl, feedAddress, chainId, maxAgeSeconds, allowStale });
  return {
    feeWei: convertUsdCentsToWei({ usdCents, ethUsdPrice: price.ethUsdPrice }),
    source: price.stale ? 'chainlink-stale' : 'chainlink',
