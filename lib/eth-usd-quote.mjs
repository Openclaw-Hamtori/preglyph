...
export function resolveChainlinkFeedAddress(feedAddress) {
  const normalizedFeedAddress = String(feedAddress || '').trim();
  return normalizedFeedAddress || DEFAULT_CHAINLINK_ETH_USD_FEED;
}
...
export async function fetchChainlinkEthUsdPrice({ rpcUrl, feedAddress = DEFAULT_CHAINLINK_ETH_USD_FEED, maxAgeSeconds = DEFAULT_FEE_QUOTE_TTL_SECONDS }) {
  if (!rpcUrl) {
  ...
  try {
    const feed = new Contract(resolveChainlinkFeedAddress(feedAddress), CHAINLINK_AGGREGATOR_V3_ABI, provider);
    ...
    return {
      ...
      feedAddress: resolveChainlinkFeedAddress(feedAddress),
    };
  } finally {
    provider.destroy?.();
  }
...
export async function getWriteFeeQuote({ rpcUrl, usdCents, overrideWei = '', feedAddress = DEFAULT_CHAINLINK_ETH_USD_FEED, maxAgeSeconds = DEFAULT_FEE_QUOTE_TTL_SECONDS }) {
  if (overrideWei) {
    return {
  ...
  const price = await fetchChainlinkEthUsdPrice({ rpcUrl, feedAddress, maxAgeSeconds });
  return {
