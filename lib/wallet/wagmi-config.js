...
import { resolveMetaMaskProvider } from './metamask-connector.mjs';
...
function buildTargetChain() {
  if (TARGET_CHAIN_ID === base.id) return base;
  ...
  return {
    ...
    testnet: ![base.id, mainnet.id].includes(TARGET_CHAIN_ID),
  };
}

export const preglyphTargetChain = buildTargetChain();
export const wagmiChains = [mainnet, base, preglyphTargetChain].filter(
...
export const wagmiConfig = createConfig({
    ...
    injected({
      target: {
        ...
        provider(windowObject) {
          return resolveMetaMaskProvider(windowObject?.ethereum);
        },
      },
    }),
  ],
  ...
  ssr: false,
  // Let wagmi persist connector state in the browser so reconnectAsync() can actually restore
  // remembered sessions after refresh. Fall back to noopStorage only outside the browser.
