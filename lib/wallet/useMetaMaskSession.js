...
export function useMetaMaskSession() {
  ...
  async function connect() {
    ...
    try {
      ...
      const hadConnectedSession = Boolean(
        connectionAddressRef.current || walletAddressRef.current || previousConnectedAddressRef.current,
      );
      const shouldReconcileAuthorizedSession = shouldReconcileConnectAfterError({
        error,
        errorDetail: connectErrorDetail.message || error?.message || connectErrorDetail.detail || '',
        hadConnectedSession,
      });

      if (shouldReconcileAuthorizedSession) {
        const reconciledAccounts = await getAuthorizedAccounts(provider);
        const reconciledAddress = reconciledAccounts?.[0] || connectionAddressRef.current || walletAddressRef.current || '';
        if (reconciledAddress) {
          ...
          setWalletDebug((current) => ({
            ...
            lastEthAccounts: reconciledAccounts.length ? reconciledAccounts : [reconciledAddress],
            lastErrorCode: '',
            lastErrorMessage: '',
            lastErrorDetail: '',
  ...
  async function disconnect() {
    ...
    const provider = activeProviderRef.current || resolveMetaMaskProvider();

    try {
      await disconnectAsync();
    } finally {
      await revokeMetaMaskPermissions(provider);
