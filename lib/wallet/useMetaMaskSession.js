...
export function useMetaMaskSession() {
  ...
  useEffect(() => {
    ...
    const syncWalletState = async (reason = 'mount') => {
      ...
      if (!provider || connectInFlightRef.current || manualDisconnectRef.current) return;
    ...
    const setupWallet = async () => {
      ...
      const handleProviderConnect = () => {
        ...
        if (!manualDisconnectRef.current && shouldRestoreOnConnect) {
          appendProbeTrace('event:connect', formatProviderSnapshot(readProviderSnapshot(metamaskProvider)));
          scheduleSync('restore:provider_connect', 0);
        }
      };
      ...
      if (shouldRestoreRemembered) {
        ...
        scheduleRestoreProbe(getRestoreProbeDelayMs({ mobile: isProbablyMobile() }));
      } else {
        setConnectionStatus('disconnected');
        setWalletProbeDone(true);
      }
