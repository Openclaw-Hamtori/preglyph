...
export function useMetaMaskSession() {
  ...
  useEffect(() => {
    connectionAddressRef.current = connection.address || '';
  }, [connection.address]);
  ...
  useEffect(() => {
    ...
    async function initializeWalletSession() {
      ...
      const shouldRestoreRememberedSession = shouldAutoReconnectWallet();
      const provider = await waitForMetaMaskProvider({
        timeoutMs: 2500,
        retryTimeoutMs: shouldRestoreRememberedSession ? 2500 : 0,
        retryDelayMs: shouldRestoreRememberedSession ? 250 : 0,
      });
      if (cancelled) return;

      const previousActiveProvider = activeProviderRef.current;
      activeProviderRef.current = provider;
      if (!provider) {
        if (shouldDeferNoProviderDisconnect({
          connectionAddress: connectionAddressRef.current,
          walletAddress: walletAddressRef.current,
          hasActiveProvider: Boolean(previousActiveProvider),
        })) {
          setWalletProbeDone(true);
          setWalletDebug((current) => ({
            ...current,
            probeStatus: current.probeStatus === 'connected' ? 'connected' : current.probeStatus,
            lastEvent: current.lastEvent === 'init' ? 'probe:deferred:no_provider' : current.lastEvent,
          }));
          appendProbeTrace('probe:deferred:no_provider');
          return;
        }
      ...
      try {
        ...
        if (!restored) {
          const authorizedAccounts = await getAuthorizedAccounts(provider);
          if (cancelled) return;
          const authorizedAddress = authorizedAccounts?.[0] || '';
          if (authorizedAddress) {
            rememberWalletConnected(undefined, authorizedAddress, METAMASK_WAGMI_CONNECTOR_ID);
            setWalletAddress(authorizedAddress);
            ...
            setWalletDebug((current) => ({
              ...
              providerDetected: true,
              providerRdns: provider?.providerInfo?.rdns || 'io.metamask?',
              selectedAddress: provider?.selectedAddress || authorizedAddress,
              chainId: provider?.chainId || current.chainId || '',
              probeStatus: 'connected',
              lastEthAccounts: authorizedAccounts,
              lastErrorCode: '',
              lastErrorMessage: '',
              lastErrorDetail: '',
              lastEvent: 'reconnect:authorized-session',
            }));
            appendProbeTrace('reconnect:authorized-session', authorizedAddress);
            return;
          }
        ...
        if (authorizedAddress) {
          ...
          setWalletDebug((current) => ({
            ...
            lastErrorMessage: '',
            lastErrorDetail: '',
            lastEvent: 'reconnect:authorized-after-error',
          }));
          appendProbeTrace('reconnect:authorized-after-error', authorizedAddress);
          return;
        }
        setConnectionStatus('disconnected');
        setWalletProbeDone(true);
  ...
  useConnectionEffect({
    onConnect(data) {
      ...
      void (async () => {
        ...
        const normalizedProvider = resolveReconnectProvider({ detectedProvider: provider, cachedProvider: activeProviderRef.current });
  ...
  async function connect() {
    ...
    const provider = resolveReconnectProvider({
      detectedProvider: await waitForMetaMaskProvider({ timeoutMs: 2500 }),
      cachedProvider: activeProviderRef.current,
    });
    ...
    try {
      ...
      const requestedAddress = requestedAccounts?.[0] || '';
      if (!requestedAddress) {
        const error = new Error('MetaMask did not return an account.');
        error.code = 'NO_ACCOUNT';
        throw error;
      }

      rememberWalletConnected(undefined, requestedAddress, METAMASK_WAGMI_CONNECTOR_ID);
      setWalletAddress(requestedAddress);
      setConnectionStatus('connected');
      setWalletProbeDone(true);
      setWalletDebug((current) => ({
        ...current,
        providerDetected: true,
        providerRdns: provider?.providerInfo?.rdns || 'io.metamask?',
        selectedAddress: provider?.selectedAddress || requestedAddress,
        chainId: provider?.chainId || current.chainId || '',
        probeStatus: 'connected',
        lastEthAccounts: requestedAccounts,
        lastPermissions: [],
        lastErrorCode: '',
        lastErrorMessage: '',
        lastErrorDetail: '',
      ...
      appendProbeTrace('connect:requestAccounts-success', requestedAddress);
      return requestedAddress;
    } catch (error) {
      const connectErrorDetail = extractMetaMaskErrorDetail(error);

      if (error?.code !== 4001 && error?.code !== -32002) {
        const reconciledAccounts = await getAuthorizedAccounts(provider);
        const reconciledAddress = reconciledAccounts?.[0] || '';
        if (reconciledAddress) {
          rememberWalletConnected(undefined, reconciledAddress, METAMASK_WAGMI_CONNECTOR_ID);
          setWalletAddress(reconciledAddress);
          setConnectionStatus('connected');
          setWalletProbeDone(true);
          setWalletDebug((current) => ({
            ...current,
            providerDetected: true,
            providerRdns: provider?.providerInfo?.rdns || 'io.metamask?',
            selectedAddress: provider?.selectedAddress || reconciledAddress,
            chainId: provider?.chainId || current.chainId || '',
            probeStatus: 'connected',
            lastEthAccounts: reconciledAccounts,
            lastErrorCode: '',
            lastErrorMessage: '',
            lastErrorDetail: '',
            lastEvent: 'connect:reconciled-after-error',
          }));
          appendProbeTrace('connect:reconciled-after-error', reconciledAddress);
