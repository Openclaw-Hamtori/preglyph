...
function isUsableEip1193Provider(provider) {
  return Boolean(provider && typeof provider.request === 'function' && typeof provider.on === 'function');
}

export function resolveMetaMaskProvider(injected = typeof window === 'undefined' ? null : window.ethereum) {
  if (!injected) return null;

  if (isPreferredMetaMaskProvider(injected) && isUsableEip1193Provider(injected)) {
    return injected;
  }
  ...
  return providers.find((provider) => isPreferredMetaMaskProvider(provider) && isUsableEip1193Provider(provider)) || null;
}
