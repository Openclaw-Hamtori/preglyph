...
export async function restoreMetaMaskSession(provider, { attempts = 2, delayMs = 350 } = {}) {
  const accounts = await requestWithRetry(provider, 'eth_accounts', { attempts, delayMs });
  return {
    accounts: accounts || [],
    address: accounts?.[0] || '',
  };
...
export async function connectMetaMask(
  provider,
  {
    ...
    recoverAuthorizedAccounts = async () => await restoreMetaMaskSession(provider, { attempts: 2, delayMs: 350 }),
    requestInteractiveAccounts = async () => await provider.request({ method: 'eth_requestAccounts' }),
    interactiveRetryAttempts = 2,
  } = {},
) {
  const boundedInteractiveRetryAttempts = Math.min(2, Math.max(1, Number(interactiveRetryAttempts) || 1));
  ...
  for (let attempt = 1; attempt <= boundedInteractiveRetryAttempts; attempt += 1) {
    try {
      ...
      const canRetryInteractive = isTransientMetaMaskStartupError(error) && attempt < boundedInteractiveRetryAttempts;
      if (!canRetryInteractive) {
        throw error;
      }
      ...
      for (let fallbackAttempt = 1; fallbackAttempt <= 2; fallbackAttempt += 1) {
        try {
          const fallbackSession = await recoverAuthorizedAccounts();
          if (fallbackSession.address) {
            return {
