...
export async function connectMetaMask(
  ...
  for (let attempt = 1; attempt <= boundedInteractiveRetryAttempts; attempt += 1) {
    try {
      ...
      try {
        const fallbackAccounts = await requestAuthorizedAccounts();
        if (fallbackAccounts?.[0]) {
          return {
            accounts: fallbackAccounts,
            address: fallbackAccounts[0],
            reusedExisting: true,
            hadTransientPreflightError,
          };
        }
      } catch {
        // Ignore fallback read failures and continue to the bounded interactive retry.
      }
    }
  }
