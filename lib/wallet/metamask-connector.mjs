...
export function shouldReconcileConnectAfterError({
  error = null,
  errorDetail = '',
  hadConnectedSession = false,
} = {}) {
  if (!hadConnectedSession) return false;
  if (error?.code === 4001 || error?.code === -32002) return false;
  return /\.on is not a function/i.test(errorDetail || error?.message || '');
}

export function resolveReconnectProvider({ detectedProvider = null, cachedProvider = null } = {}) {
  const normalizedDetectedProvider = resolveMetaMaskProvider(detectedProvider);
  if (normalizedDetectedProvider) return normalizedDetectedProvider;

  const normalizedCachedProvider = resolveMetaMaskProvider(cachedProvider);
  if (normalizedCachedProvider) return normalizedCachedProvider;
...
function normalizeEip1193Provider(provider) {
  ...
  // Some MetaMask wrappers/proxies still provide a working request() surface but do not
  // mirror event emitter helpers directly on the same object. Keep the provider usable for
  // connect flows instead of misclassifying MetaMask as missing.
  return createDelegatingProvider(provider, () => {});
}
...
function hasProviderEventApi(provider) {
  return typeof provider?.on === 'function' || typeof provider?.addListener === 'function';
}
...
export function resolveMetaMaskProvider(injected = typeof window === 'undefined' ? null : window.ethereum) {
  ...
  if (isPreferredMetaMaskProvider(injected) && hasProviderEventApi(injected)) {
    const normalizedProvider = normalizeEip1193Provider(injected);
    if (normalizedProvider) return normalizedProvider;
  }

  const byRdns = providers.find((provider) => (
    provider?.providerInfo?.rdns === 'io.metamask' &&
    hasProviderEventApi(provider) &&
    isUsableEip1193Provider(provider)
  ));
  if (byRdns) return normalizeEip1193Provider(byRdns);

  const preferredProvider = providers.find((provider) => (
    isPreferredMetaMaskProvider(provider) &&
    hasProviderEventApi(provider) &&
    isUsableEip1193Provider(provider)
  ));
  if (preferredProvider) return normalizeEip1193Provider(preferredProvider);
  ...
  const byRdnsFallback = providers.find((provider) => provider?.providerInfo?.rdns === 'io.metamask' && isUsableEip1193Provider(provider));
  if (byRdnsFallback) return normalizeEip1193Provider(byRdnsFallback);

  const preferredProviderFallback = providers.find((provider) => isPreferredMetaMaskProvider(provider) && isUsableEip1193Provider(provider));
  return preferredProviderFallback ? normalizeEip1193Provider(preferredProviderFallback) : null;
}
...
async function waitForSingleMetaMaskProviderAttempt({
  windowObject = typeof window === 'undefined' ? null : window,
  timeoutMs = 1200,
...
export async function waitForMetaMaskProvider({
  ...
  const provider = await waitForSingleMetaMaskProviderAttempt({
    windowObject,
    timeoutMs,
    getProvider,
  });
  if (provider || !retryTimeoutMs || !windowObject) return provider;

  if (retryDelayMs > 0) {
    await new Promise((resolve) => setTimeout(resolve, retryDelayMs));
  }

  return await waitForSingleMetaMaskProviderAttempt({
    windowObject,
...
export async function getAuthorizedAccounts(provider) {
...
export async function revokeMetaMaskPermissions(provider) {
  if (!provider || typeof provider.request !== 'function') {
    return false;
  }
